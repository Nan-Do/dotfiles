...
# Function to remove the last x commands
function forget_last_commands
    set -l count $argv[1]
    if test -z "$count"
        echo "Usage: forget_last_commands <number_of_commands>"
        return 1
    end

    for cmd in (builtin history search --max $count)
